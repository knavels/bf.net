...
internal abstract class IlGenerator
{
        ...
        List<string> referencePaths, string fullPathToSourceCode)
    {
        ...
        var debuggableAttributeType = FindRuntimeType(systemRuntime, "System.Diagnostics.DebuggableAttribute");
        ...
        mainMethod.Body.Variables.Add(new VariableDefinition(module.TypeSystem.Boolean)); //3 comparison
    ...
    private static void EmitConditionalJump(ILProcessor il, IDictionary<int, string> instructionsToFix,
        string targetLabelName, Document document, IInstruction instruction)
    {
        AddSequencePoint(il, document, instruction, theIl =>
        {
            theIl.Emit(OpCodes.Ldloc_0);
            theIl.Emit(OpCodes.Ldloc_1);
            theIl.Emit(OpCodes.Ldelem_U1);
            theIl.Emit(OpCodes.Ldc_I4_0);
            theIl.Emit(OpCodes.Ceq);
            theIl.Emit(OpCodes.Stloc_3);
            theIl.Emit(OpCodes.Ldloc_3);

            instructionsToFix.Add(theIl.Body.Instructions.Count, targetLabelName);
            theIl.Emit(OpCodes.Brtrue, Instruction.Create(OpCodes.Nop));
        });
    }
    ...
    private static void EmitMove(ILProcessor il, int quantity, Document document, IInstruction instruction)
    {
        AddSequencePoint(il, document, instruction, theIl =>
        {
            ...
            theIl.Emit(OpCodes.Ldc_I4, Math.Abs(quantity));
            theIl.Emit(quantity > 0 ? OpCodes.Add : OpCodes.Sub);
        ...
        IInstruction instruction)
    {
        AddSequencePoint(il, document, instruction, theIl =>
        {
            theIl.Emit(OpCodes.Ldloc_0);
            theIl.Emit(OpCodes.Ldloc_1);
            theIl.Emit(OpCodes.Ldelema, module.TypeSystem.Byte);
            theIl.Emit(OpCodes.Dup);
            theIl.Emit(OpCodes.Ldind_U1);

            theIl.Emit(OpCodes.Ldc_I4, Math.Abs(quantity));
            theIl.Emit(quantity > 0 ? OpCodes.Add : OpCodes.Sub);

            theIl.Emit(OpCodes.Conv_U1);
            theIl.Emit(OpCodes.Stind_I1);
        });
    }
        ...
        Document document, IInstruction instruction)
    {
        AddSequencePoint(il, document, instruction, theIl =>
        {
            ...
            theIl.Emit(OpCodes.Ldelema, module.TypeSystem.Byte);
            theIl.Emit(OpCodes.Ldind_U1);
            theIl.Emit(OpCodes.Call, writeMethod);
        });
    }
        ...
        ILProcessor il, Document document, IInstruction instruction)
    {
        AddSequencePoint(il, document, instruction, theIl =>
        {
            theIl.Emit(OpCodes.Ldloc_0);
            theIl.Emit(OpCodes.Ldloc_1);
            theIl.Emit(OpCodes.Call, readKeyMethod);
            theIl.Emit(OpCodes.Stloc_2);
            theIl.Emit(OpCodes.Ldloca, 2);
            theIl.Emit(OpCodes.Call, keyMethod);
            theIl.Emit(OpCodes.Conv_U1);
            theIl.Emit(OpCodes.Stelem_I1);
        });
    }
    ...
    private static TypeDefinition FindRuntimeType(AssemblyDefinition assemblyDefinition, string fullName)
    {
        var type = assemblyDefinition.Modules.SelectMany(m => m.ExportedTypes)
